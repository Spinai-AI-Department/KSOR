...
from app.db.session import get_db_auth as get_db_survey
...
async def survey_status(token_uuid: UUID, conn=Depends(get_db_survey)):
...
async def survey_verify(token_uuid: UUID, payload: SurveyVerifyRequest, conn=Depends(get_db_survey)):
...
async def survey_questions(token_uuid: UUID, claims=Depends(require_survey_token), conn=Depends(get_db_survey)):
...
async def survey_save(token_uuid: UUID, payload: SurveySaveRequest, claims=Depends(require_survey_token), conn=Depends(get_db_survey)):
...
async def survey_submit(token_uuid: UUID, payload: SurveySubmitRequest, claims=Depends(require_survey_token), conn=Depends(get_db_survey)):
